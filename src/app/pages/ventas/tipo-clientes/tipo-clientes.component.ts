...
@Component({
  ...
  styleUrls: ['./tipo-clientes.component.css']
})
export class TipoClientesComponent implements OnInit {
 ...
 constructor(public _service: TipoClientesService,
  ...
  this._service.mostrarpermiso(localStorage.getItem('rol'),20);
     ...
     then((result) => {
       ...
       if (result) {
         this._service.eliminar(id).subscribe(resp => {
           this._service.mostrar();
           if (!resp.ok) {
             this._sweet.mensajeSimple('Ocurrio un error', 'TIPO CLIENTES', 'error');
           } else {
            let params = {
              operacion: 'ELIMINO',
              fecha: new Date(),
            ...
            this._bitacora.crear(params).subscribe();
             this._sweet.mensajeSimple('Eliminado correctamente', 'TIPO CLIENTES', 'success');
           }
         })
       }
     })
